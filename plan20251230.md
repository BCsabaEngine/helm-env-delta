...
1. Update `mergeTransformRules` to merge file arrays:
   ...
   const mergeTransformRules = (parent?, child?): TransformRules => {
     ...
     return {
       ...
       contentFile: [...normalizeToArray(parent?.contentFile), ...normalizeToArray(child?.contentFile)],
       filenameFile: [...normalizeToArray(parent?.filenameFile), ...normalizeToArray(child?.filenameFile)]
     };
   };
   ```
